...
import (
	...
	"github.com/lzkking/edr/plugins/collect/utils"
	...
	for _, dir := range SearchDir {
		godirwalk.Walk(dir, &godirwalk.Options{
			Callback: func(osPathname string, directoryEntry *godirwalk.Dirent) error {
				if strings.HasSuffix(directoryEntry.Name(), ".service") && (directoryEntry.IsRegular() || directoryEntry.IsSymlink()) && !set.Contains(directoryEntry.Name()) {
					if f, err := os.Open(osPathname); err == nil {
						set.Add(directoryEntry.Name())
						defer f.Close()
						...
						u.Checksum, _ = utils.GetMd5(osPathname, "")
						u.SetDefault()

						rec := &plugins.Record{
							DataType:  int32(h.DataType()),
							Timestamp: time.Now().Unix(),
							Data:      &plugins.Payload{Fields: make(map[string]string, 7)},
						}
